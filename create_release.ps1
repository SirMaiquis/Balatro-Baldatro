...
$zipFileName = Join-Path $versionDir "Baldatro-$cleanVersionName.zip"
$zipFileNameTS = Join-Path $versionDir "Baldatro-$cleanVersionName-TS.zip"

Write-Host "Creating releases in: $versionDir" -ForegroundColor Green
Write-Host "  1. Baldatro-$cleanVersionName.zip (mod only)" -ForegroundColor Cyan
Write-Host "  2. Baldatro-$cleanVersionName-TS.zip (full package)" -ForegroundColor Cyan
...
try {
    ...
    # Create Baldatro folder inside temp directory
    $baldatroFolder = Join-Path $tempDir1 "Baldatro"
    New-Item -ItemType Directory -Path $baldatroFolder -Force | Out-Null
    ...
    # Copy assets folder
    if (Test-Path "assets") {
        Copy-Item -Path "assets" -Destination $tempDir2 -Recurse -Force
    }
...
Write-Host "  * Baldatro-$cleanVersionName.zip (mod only)" -ForegroundColor White
Write-Host "  * Baldatro-$cleanVersionName-TS.zip (full package)" -ForegroundColor White
Write-Host ""
Read-Host "Press Enter to exit" 